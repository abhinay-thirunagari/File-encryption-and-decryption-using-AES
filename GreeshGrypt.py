...
class MainWindow:
    ...
    def __init__(self, root):
        ...
        root.title("GreeshCrypt")
        root.configure(bg="#F4F2F0")

        try:
            icon_img = tk.Image(
                "photo", file=self.THIS_FOLDER_G + "GreeshCrypt.ico"
            )
            root.call("wm", "iconphoto", root._w, icon_img)
        except Exception:
            pass

        self.menu_bar = tk.Menu(root, bg="#F4F2F0", relief=tk.FLAT)
        self.menu_bar.add_command(
            label="Help!", command=self.show_help_callback)
        ...
        self.file_entry_label = tk.Label(
            root,
            text="Enter the path of the file or click CHOOSE FILE to select a file",
            bg="#F4F2F0",
            anchor=tk.W,
        )
        ...
        self.key_entry_label1 = tk.Label(
            root,
            text="Enter Key (Keep It Safe - Needed For Decryption)",
            bg="#F4F2F0",
            anchor=tk.W,
        )
        ...
        self.key_entry_label2 = tk.Label(
            root, text="Re-enter Secret Key", bg="#F4F2F0", anchor=tk.W
        )
    ...
    def unfreeze_controls(self):
        ...
        self.select_btn.configure(state="normal", bg="#00A170")
        self.encrypt_btn.configure(state="normal", bg="#02569B")
        self.decrypt_btn.configure(state="normal", bg="#02569B")
    ...
    def e_check_callback(self):
        ...
        if newPath.is_file():
            ...
            messagebox.showinfo(
                "GreeshCrypt", "Please enter a valid file path!")
            return

        if len(self._secret_key.get()) == 0:
            messagebox.showinfo(
                "GreeshCrypt", "Secret key should be more than 8 charaters!")
            return
        elif self._secret_key.get() != self._secret_key_check.get():
            messagebox.showinfo("GreeshCrypt", "Secret keys do not match!")
            return
    ...
    def d_check_callback(self):
        ...
        if self._file_url.get()[-4:] != "encr":
            messagebox.showinfo(
                "GreeshCrypt",
                """Please provide an encrypted file!""",
            )
            return

        if len(self._secret_key.get()) == 0:
            messagebox.showinfo("GreeshCrypt", "Please enter a secret key!")
            return
        elif self._secret_key.get() != self._secret_key_check.get():
            messagebox.showinfo("GreeshCrypt", "Secret keys do not match")
            return
    ...
    def encrypt_execute(self):
        ...
        try:
            ...
            if self.should_cancel:
                self._cipher.abort()
                self._status.set("Cancelation done successfully!")
                messagebox.showinfo(
                    "GreeshCrypt", "Cancelation done successfully!")
                self._cipher = None
                self.should_cancel = False
            ...
            self._cipher = None
            self.should_cancel = False
            self._status.set("File encrypted successfully!")
            messagebox.showinfo("GreeshCrypt", "File encrypted successfully!")
        except Exception as e:
    ...
    def decrypt_execute(self):
        ...
        try:
            ...
            self._cipher = None
            self.should_cancel = False
            self._status.set("File decyption done successfully!")
            messagebox.showinfo(
                "GreeshCrypt", "File decyption done successfully!")
        except Exception as e:
    ...
    def show_help_callback(self):
        messagebox.showinfo(
            "Help",
            """1. Open the Application and Click CHOOSE FILE Button to select your file e.g. "mydoc.pdf" (OR You can add path manually).
...
3. Click Encrypt File Button to encrypt the file. A new encrypted file with ".encr" extention e.g. "mydoc.pdf.encr" will be created in the same directory where the "mydoc.pdf" is.
4. When you want to Decrypt a file you, will select the file with the ".encr" extention and Enter your Key which you chose at the time of Encryption. Click Decrypt File Button to decrypt. The decrypted file will be of the same name as before with the suffix "decrypted" for e.g. "mydoc_decrypted.pdf".
5. Click CLEAR Button to reset the input fields and status bar.""",
    ...
    def show_about(self):
        messagebox.showinfo(
            "GreeshCrypt v1.1.0",
            """GreeshCrypt is a File Encryption Tool based on AES Algorithm. 
Managed by Greeshma.
""",
        )
...
if __name__ == "__main__":
    ...
    path_to_ico = os.path.abspath(os.path.join(bundle_dir, "GreeshCrypt.ico"))
